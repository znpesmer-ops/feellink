    ...
    async mainLoop(client, bclient) {
        ...
        while ((!this.closing && !this.paused) || asyncFifoQueue.numTotal() > 0) {
            ...
            if (job) {
                ...
                asyncFifoQueue.add(this.processJob(job, token, () => asyncFifoQueue.numTotal() <= this._concurrency, jobsInProgress));
            }
        ...
        return this.trace(SpanKind.CONSUMER, 'process', this.name, async (span) => {
            ...
            try {
                ...
                if (unrecoverableErrorMessage) {
                    const failed = await this.retryIfFailed(() => this.handleFailed(new UnrecoverableError(unrecoverableErrorMessage), job, token, fetchNextCallback, jobsInProgress, inProgressItem, span), { delayInMs: this.opts.runRetryDelay, span });
                    return failed;
                }
                jobsInProgress.add(inProgressItem);
                const result = await this.callProcessJob(job, token);
                return await this.retryIfFailed(() => this.handleCompleted(result, job, token, fetchNextCallback, jobsInProgress, inProgressItem, span), { delayInMs: this.opts.runRetryDelay, span });
            }
            catch (err) {
                const failed = await this.retryIfFailed(() => this.handleFailed(err, job, token, fetchNextCallback, jobsInProgress, inProgressItem, span), { delayInMs: this.opts.runRetryDelay, span, onlyEmitError: true });
                return failed;
            }
            finally {
                span === null || span === void 0 ? void 0 : span.setAttributes({
